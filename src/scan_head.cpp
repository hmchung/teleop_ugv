...
int main(int argc, char** argv){
	...
	nh_param.param<double>("/scan_angle_lower_input", angle_lower_input, angle_lower_input);
	nh_param.param<double>("/scan_angle_upper_input", angle_upper_input, angle_upper_input);
	...
	ros::Publisher tilt_angle_pub = nh.advertise<std_msgs::UInt8>(topicName_pub_tiltAngle, 10);
	...
	while (ros::ok()){
		if (scanning_mode == true){
			...
			angle_msg.data = (int)(current_target_angle / PI * 180) + 90;
