...
int control_rate = 2;
int loop_count = 0;
double lin_accel = 0.0, ang_accel = 0.0;
...
double v_lin_x, v_lin_y, v_rot_z;
double allowable_vel_diff = 1.0;

double wheel_rad = 0.075; //m
...
void calculate_v_wheels(){
	if (enc_has_data){
		...
		double v_x_FL, v_x_FR, v_x_BR, v_x_BL; 
		double v_y_FL, v_y_FR, v_y_BR, v_y_BL;

		v_x_FL = v_wheel_FL * 0.5;
		v_y_FL = v_wheel_FL * 0.5;

		v_x_FR = v_wheel_FR * 0.5;
		v_y_FR = v_wheel_FR * -0.5;

		v_x_BR = v_wheel_BR * 0.5;
		v_y_BR = v_wheel_BR * 0.5;

		v_x_BL = v_wheel_BL * 0.5;
		v_y_BL = v_wheel_BL * -0.5;

		v_lin_x = (v_x_FL + v_x_FR + v_x_BR + v_x_BL) / 4.0;
		v_lin_y = (v_y_FL + v_y_BR + v_y_FR + v_y_BL) / 4.0;
		v_rot_z = 	((v_x_FR + v_x_BR - v_x_FL - v_x_BL) * robot_w / 2.0 + 
					((v_y_FL + v_y_FR - v_y_BR - v_y_BL) * robot_l / 2.0))/ 4.0;
...
void publish_wheelVel(){
	if (enc_has_data) {
		...
		tempTwist.twist.linear.x = v_lin_x;
		tempTwist.twist.linear.y = v_lin_y;
		tempTwist.twist.angular.z = v_rot_z;
...
void prepare_raw_output(){
	double motor_vals[4];
	double control_speed_x = target_vel.linear.x;
